...
{
  lnipca12003_2010<-window(lnipca1, frequency=12, start=c(2003,1), end=c(2010,1))
  lnhiato12003_2010<-window(lnhiato1, frequency=12, start=c(2003,1), end=c(2010,1))
  lnCambio12003_2010<-window(lnCambio1, frequency=12, start=c(2003,1), end=c(2010,1))
...
{
  lnipca12003_2010<-window(lnipca1, frequency=12, start=c(2003,1), end=c(2010,1))
  lneipca12003_2010<-window(lneipca1, frequency=12, start=c(2003,1), end=c(2010,1))
  lnhiato12003_2010<-window(lnhiato1, frequency=12, start=c(2003,1), end=c(2010,1))
  lnCambio12003_2010<-window(lnCambio1, frequency=12, start=c(2003,1), end=c(2010,1))
              ...
              #IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII#
{
                ...
                #DIFF Só no cambio (N AJEITEI)
                {
                  Dlnipca1=diff(lnipca1);plot(lnipca1)
                  Dlnhiato1=diff(lnhiato1);plot(lnhiato1)
                  DlnCambio1=diff(lnCambio1);plot(lnCambio1)
                  PhillipsBPotDiff=dynlm(lnipca1~lag(lnipca1,-1)+lag(lnipca1,-2)+lag(lnhiato1,-1)+DlnCambio1)
              ...
              #IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII#
{
  ...
  #TESTE HIATO1
  Dipca=diff(ipca);plot(Dipca)
  Dhiato=diff(hiato1);plot(Dhiato)
  DCambio=diff(Cambio);plot(DCambio)
  Deipca=diff(eipca);plot(Deipca)
  PhillipsFLinDiff=dynlm(Dipca~lag(Dipca,-1)+Deipca+lag(Dhiato,-1)+DCambio)
  summary(PhillipsFLinDiff)
...
#DIFF Só no cambio
{
  ...
  PhillipsFPotDiff=dynlm(lnipca1~lag(lnipca1,-1)+lneipca1+lag(lnhiato1,-1)+DlnCambio1)
  summary(PhillipsFPotDiff)
  regressao=PhillipsFPotDiff
  variavel=lnCambio1
  name='lnCambio1'
  Var_Independente1=lnipca1
  name1='Dlnipca1 Defasado em 1'
  Var_Independente2=lneipca1
  name2='Dlneipca1'
  Var_Independente3=lnhiato1
  name3='Dlnhiato1 Defasado em 1'
  Var_Independente4=DlnCambio1
  name4='DlnCambio1'
}
...
# GRAFICO IPCA X CAMBIO
p1<-ggplot(data=PhillipsGraph, aes (x= Data))
p1<-p1+geom_line(size=1.2, aes(y=ipca, colour="blue"))
p1<-p1+geom_line(size=1.2, aes(y=Cambio, colour="red"))
p1<-p1+scale_color_manual(name="Variáveis"
                          ,labels= c("IPCA", "Cambio")
                          , values=c("violetred1", "dodgerblue3"))
p1<-p1+labs(title = "IPCA x Cambio", x= "Data",y="R$ mi",subtitle = "2003.1 - 2021.12")+
  theme(axis.title.x= element_text(size=13),
        axis.title.y=element_text(size=13),
        axis.text.x=element_text(size=12),
        axis.text.y = element_text(size=12),
